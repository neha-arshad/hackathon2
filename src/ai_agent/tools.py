...
# Environment variables for login (multi-user can pass these dynamically)
AUTH_EMAIL = os.getenv("AUTH_EMAIL")
AUTH_PASSWORD = os.getenv("AUTH_PASSWORD")

# Temporary in-memory cache for user tokens
# In production, replace with session store or DB
...
def login_user(email: str, password: str) -> Dict:
    ...
    try:
        ...
        if response.status_code != 200:
            return {"error": {"detail": f"Login failed: {response.text}"}, "status_code": response.status_code}

        token = response.json().get("access_token")
        if not token:
            return {"error": {"detail": "Token not returned from login API"}, "status_code": 500}

        # Cache token for this user (simple in-memory)
        USER_TOKENS[email] = token
        return {"token": token}

    except Exception as e:
        return {"error": {"detail": f"Login request failed: {str(e)}"}, "status_code": 500}


def get_user_token(email: str) -> Optional[str]:
    """
    Retrieve cached token for a user
    """
    return USER_TOKENS.get(email)
...
def make_api_request(endpoint: str, method: str = "GET", data: dict = None, auth_token: str = None, timeout: int = 30):
    ...
    if not auth_token:
        return {
            "error": {"detail": "Authentication required. Please login first."},
            "status_code": 401
        }
...
# ---------------------- Tools Functions ----------------------
def add_task(params: AddTaskParams, auth_token: str) -> Dict:
    try:
        payload = {
            "title": params.title,
            "description": params.description,
...
def list_tasks(params: ListTasksParams, auth_token: str) -> Dict:
    try:
        result = make_api_request("/tasks", "GET", auth_token=auth_token, timeout=30)
        if "error" in result:
            return {"result": "Error listing tasks", "details": result["error"]}
...
def update_task(params: UpdateTaskParams, auth_token: str) -> Dict:
    try:
        payload = {}
        if params.title is not None:
            payload["title"] = params.title
...
def delete_task(params: DeleteTaskParams, auth_token: str) -> Dict:
    try:
        result = make_api_request(f"/tasks/{params.task_id}", "DELETE", auth_token=auth_token, timeout=30)
        if "error" in result:
            return {"result": f"Error deleting task {params.task_id}", "details": result["error"]}
...
def mark_task_complete(params: MarkTaskCompleteParams, auth_token: str) -> Dict:
    try:
        result = make_api_request(f"/tasks/{params.task_id}/complete", "PUT", {"completed": params.complete}, auth_token, timeout=30)
        if "error" in result:
            return {"result": f"Error updating task {params.task_id} status", "details": result["error"]}
...
# ---------------------- Tools Definitions for AI Agent ----------------------
TOOLS = [
    {
        "type": "function",
        "function": {
...
# ---------------------- Call Tool ----------------------
def call_tool(tool_name: str, tool_args: Dict, auth_token: str) -> Dict:
    """
    Call the appropriate tool based on the tool name and arguments
    """
    if not auth_token:
        return {"result": "Authentication required", "details": "Please login first to continue"}
